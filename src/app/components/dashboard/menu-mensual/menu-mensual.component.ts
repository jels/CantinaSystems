...
export class MenuMensualComponent implements OnInit {
  ...
  agregarPlatoMensual() {
    ...
    this._almuerzoService.getCantidadMenusDiario((this.form.value.fechaAlmuerzo.getDate() + "/" + (this.form.value.fechaAlmuerzo.getMonth() + 1) + "/" + this.form.value.fechaAlmuerzo.getFullYear())).subscribe(res => {
      this.cantidadAlmuerzoDiario = res;
